...
const LOCAL_STORAGE_BOOKMARKS = "news&blogs_bookmarks";
...
function News({ onShowBlogs, blogs, onEditBlog, onDeleteBlog }) {
  ...
  useEffect(() => {
    const savedBookmarks =
      JSON.parse(localStorage.getItem(LOCAL_STORAGE_BOOKMARKS)) || [];
    ...
    const fetchNews = async () => {
      ...
      try {
        ...
        if (error.response && error.response.status === 403) {
          setHeadline(null);
          setNews([]);
          // You'll need to create this state variable to show the error message
          setError(
            <p>
  ...
  const handleBookmarkClick = (article) => {
    setBookmarks((prevBookmarks) => {
      ...
      const updatedBookmarks = prevBookmarks.find(
        ...
        ? // If found, remove it (unbookmark)
          prevBookmarks.filter((bookmark) => bookmark.title !== article.title)
        : // If not found, add it (bookmark)
